...
public class LoanApplicationServiceImpl implements LoanApplicationService {
    ...
    public LoanApplicationResponse applyLoan(LoanApplicationRequest request) {
        ...
        if (request.getDownPayment().add(request.getLoanAmount()).compareTo(vehicle.getOnRoadPrice()) < 0) {
            throw new InvalidLoanApplicationException("Down payment plus loan amount must cover vehicle on-road price");
        }
    ...
    public List<LoanApplicationResponse> getLoansByCustomerId(Long customerId) {
        return loanApplicationRepository.findByCustomerId(customerId).stream().map(LoanApplicationMapper::toResponse).toList();
    }

    @Override
    public List<LoanApplicationResponse> getAllLoans() {
        return loanApplicationRepository.findAll().stream().map(LoanApplicationMapper::toResponse).toList();
    }
