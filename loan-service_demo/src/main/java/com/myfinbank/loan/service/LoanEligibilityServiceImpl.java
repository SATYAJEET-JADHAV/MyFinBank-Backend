...
public class LoanEligibilityServiceImpl implements LoanEligibilityService {
    ...
    public EligibilityResponse checkEligibility(LoanEligibilityRequest request) {
        ...
        BigDecimal maxEligibleAmount = vehicle.getOnRoadPrice()
                .multiply(BigDecimal.valueOf(0.80))
                .setScale(2, RoundingMode.HALF_UP);

        if (customer.getMonthlyIncome().compareTo(BigDecimal.valueOf(25000)) < 0) {
        ...
        if (age < 21 || age > 60) {
            return EligibilityResponse.builder()
                    .eligible(false)
                    .reason("Customer age must be between 21 and 60")
                    .maxEligibleAmount(maxEligibleAmount)
                    .build();
        }
        ...
        if (request.getLoanAmount().compareTo(maxEligibleAmount) > 0) {
            return EligibilityResponse.builder()
                    .eligible(false)
                    .reason("Loan amount cannot exceed 80% of vehicle price")
                    .maxEligibleAmount(maxEligibleAmount)
                    .build();
        }
        ...
        return EligibilityResponse.builder()
                .eligible(true)
                .reason("Customer is eligible for loan")
                .maxEligibleAmount(maxEligibleAmount)
                .build();
    }
}